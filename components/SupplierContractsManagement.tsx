...
const SupplierContractsManagement: React.FC<SupplierContractsManagementProps> = ({
  ...
  const generalContractColumns: { key: keyof License | 'actions' | 'remaining' | 'attachments'; header: string; render?: (item: License) => React.ReactNode; exportValue?: (item: License) => string | number | null | undefined; headerClassName?: string; cellClassName?: string; }[] = [
    ...
    { key: 'status', header: 'حالة العقد', headerClassName: baseHeaderClass, cellClassName: baseCellClass },
    { key: 'renewalType', header: 'نوع التجديد', headerClassName: baseHeaderClass, cellClassName: baseCellClass },
  ...
  return (
    <div>
      <DataTable
        title={<div className="flex items-center gap-3"><span className="text-blue-800 h-6 w-6"><SupplierIcon /></span><span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-md font-semibold">عقود الموردين</span></div>}
